...
import { importCampaignKols, tierLabel } from '../lib/campaigns'
...
// Add creators from a spreadsheet to an EXISTING campaign. Mirrors
// ImportCampaignModal's parsing/preview, minus the create-campaign form —
// rows upsert into the current campaign (dedup on campaign_id,kol_handle).
export default function ImportKolsModal({ campaignId, campaignName, existingHandles = [], onClose, onImported }) {
  ...
  const newCount = useMemo(() => included.filter((r) => !have.has(r.handle)).length, [included, have])
  ...
  const doImport = async () => {
    setError(null)
    if (included.length === 0) return setError('No creators selected to import')
    setImporting(true)
    try {
      const result = await importCampaignKols(campaignId, included)
      onImported(result)
    } catch (e) {
      setError(e.message)
      setImporting(false)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 backdrop-blur-[2px] px-4"
      ...
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label="Add creators from a spreadsheet"
        className="w-full max-w-[640px] max-h-[90vh] flex flex-col bg-white rounded-[16px] shadow-xl"
        onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-start justify-between p-6 pb-4">
          <div>
            <p className="font-mono text-[10px] tracking-[.16em] text-faint uppercase mb-1">Add creators</p>
            <h2 className="text-[18px] font-semibold text-ink">
              From a spreadsheet{campaignName ? <span className="text-muted font-normal"> · {campaignName}</span> : null}
            </h2>
          </div>
            ...
            <label className={`flex flex-col items-center justify-center py-14 border-2 border-dashed rounded-[14px] cursor-pointer transition-colors ${
              ...
              {parsing ? (
                ...
                <>
                  ...
                  <p className="text-[12px] text-muted">Export the tab with your creator list (one KOL per row)</p>
                </>
              )}
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                {missingRequired.length > 0 && (
                  <p className="text-[11px] text-rose mt-2">Map the Handle column to preview creators.</p>
                )}
              </div>
              ...
              <div className="border-t border-mist pt-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <p className="text-[11px] font-mono text-faint">
                    {newCount} new · {included.length - newCount} already on campaign · {skipped} rows skipped
                  </p>
                </div>
                {rows.length === 0 ? (
                  <p className="text-[12px] text-muted py-4 text-center">No creator rows detected — check the Handle column mapping above.</p>
                ) : (
                  <div className="max-h-[240px] overflow-y-auto space-y-1 pr-1">
                    {rows.map((r) => {
                      const on = !excluded.has(r.sheetRow)
                      const already = have.has(r.handle)
                      return (
                        <label key={r.sheetRow}
                          ...
                          {already && <span className="text-[10px] font-mono text-faint">on campaign</span>}
          ...
          <div className="flex items-center justify-between gap-2 p-6 pt-4 border-t border-mist">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={doImport} disabled={importing || included.length === 0}
                ...
                {importing ? 'Importing…' : `Add ${included.length} creator${included.length === 1 ? '' : 's'}`}
              </button>
            </div>
          </div>
        )}
